...
/* ===========================================================================
   ...
   Flappy-Prinzip: Das Datenpaket sinkt ständig, kurze Impulse (Leertaste/
   W/↑ oder Klick) halten es in der Luft. Durch die Lücken der Firewall-
   Wände fliegen — Berührung oder Boden/Decke bedeutet Paketverlust.
   =========================================================================== */
const NetGame = {
  ...
  gameOver(msg) {
    ...
    showGameOver(msg, this.score, this.best, this.passed + ' Pakete zugestellt');
  },
  update(dt) {
    ...
    if (this.spawnT <= 0) {
      ...
      this.walls.push({ x: CW + 30, w: 34, gapY, gap: this.gap, passed: false });
  ...
  draw() {
    ...
    // Firewall-Wände (gemauerte Balken)
    for (const w of this.walls) {
  ...
  infoFields() {
    return [
      ...
      { k:'Firewall-Lücke', v: Math.round(this.gap) + ' px' },
      { k:'Adaptername', v: 'Ethernet', small:true },
      { k:'IPv4-Adresse', v: '192.168.178.42', small:true },
      { k:'Verluste', v: this.state === 'over' ? '1' : '0', small:true },
